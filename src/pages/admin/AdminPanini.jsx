...
const emptyPanino = {
  nome: "", descrizione: "", foto: "", prezzo: "", ingredienti: "",
  categoria: "panino", in_evidenza: false, disponibile: true, allergeni: "",
};

export default function AdminPanini() {
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState(null);
  const [form, setForm] = useState(emptyPanino);
  ...
  const closeDialog = () => { setDialogOpen(false); setEditing(null); setForm(emptyPanino); };

  const openEdit = (p) => {
    setEditing(p);
    setForm({
      nome: p.nome || "", descrizione: p.descrizione || "", foto: p.foto || "",
      prezzo: p.prezzo || "", ingredienti: p.ingredienti || "", categoria: p.categoria || "panino",
      in_evidenza: p.in_evidenza || false, disponibile: p.disponibile !== false,
  ...
  const handleSubmit = (e) => {
    ...
    const data = {
      ...
      ingredienti: form.ingredienti ? form.ingredienti : null,
    };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-heading text-2xl font-bold">Gestione Panini</h1>
          <p className="text-muted-foreground text-sm mt-1">{panini.length} panini</p>
        </div>
        <Button onClick={() => { setForm(emptyPanino); setDialogOpen(true); }} className="rounded-lg">
          <Plus className="w-4 h-4 mr-2" /> Nuovo Panino
        </Button>
      </div>
            ...
            <div key={p.id} className="flex items-center gap-4 bg-card rounded-xl border border-border p-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {p.in_evidenza && <span className="text-[10px] px-1.5 py-0.5 bg-accent/20 text-accent-foreground rounded">★</span>}
                  {p.disponibile === false && <span className="text-[10px] px-1.5 py-0.5 bg-destructive/10 text-destructive rounded">Non disp.</span>}
                </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o) closeDialog(); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-heading">{editing ? "Modifica Panino" : "Nuovo Panino"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-2">
                ...
                <Input type="text" value={form.ingredienti} onChange={(e) => setForm({ ...form, ingredienti: e.target.value })} />
              </div>
            </div>
            <div className="space-y-2">
            ...
            <Button type="submit" className="w-full rounded-lg" disabled={createMut.isPending || updateMut.isPending}>
              {editing ? "Salva Modifiche" : "Aggiungi Panino"}
            </Button>
          </form>
        </DialogContent>
      </Dialog>
